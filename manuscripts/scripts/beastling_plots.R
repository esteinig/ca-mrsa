...
get_hpd <- function(data, ci=0.95, dig=9, science=FALSE){
    
    hpd <- getHPD(data, alpha=1-ci)
...
read_log_files <- function(
    ...
    for (run in unique(df1$run_name)){
        
        df1.run <- df1[df1$run_name == run,]
        df2.run <- df2[df2$run_name == run,]
        
        write.csv(df1.run, paste0(run, "_posterior.tsv"), sep="\t", quote=FALSE)
        write.csv(df2.run, paste0(run, "_re_hpd.tsv"), sep="\t", quote=FALSE)
        
        drops <- c("run_name")
        df1.run  <- df1.run[ , !(names(df1.run) %in% drops)]
        
        hpds <- lapply(df1.run, get_hpd, ci=hpd)
        
        hpd_data <- data.frame(hpds)
        rownames(hpd_data) <- c("lower", "median", "upper")
        
        hpd_data <- t(hpd_data)
        write.csv(hpd_data, paste0(run, "_hpd.tsv"), sep="\t", quote=FALSE)
...
get_sequential_palette <- function(palette, n){
    return(
...
create_ridge_plots <- function(df, plot_path, plot_name, infectious_limit=NULL){
    ...
    ncols <- length(unique(df$run_name))

    
    p1 <- ggplot(df, aes(x=TreeHeight, y=run_name, fill=run_name)) + ggtitle("MRCA\n") +
        geom_density_ridges(scale=1.5, alpha=1) + scale_fill_manual(values = get_sequential_palette("Purples", ncols)) + xlab("") + ylab("") + 
        theme_cowplot() + theme(
            panel.grid.minor = element_blank(),
            legend.position="none"
        ) 
    ...
    p2 <- ggplot(df, aes(x=Origin, y=run_name, fill=run_name)) + ggtitle("Origin\n") +
        geom_density_ridges(scale=1.5, alpha=1) + scale_fill_manual(values = get_sequential_palette("Blues", ncols)) + xlab("") + ylab("") +
        theme_cowplot() + theme(
            panel.grid.minor = element_blank(),
            legend.position="none"
        ) 
    ...
    if (var(df$ClockRate) == 0){
        ...
        p3 <- ggplot(df, aes(x=ClockRate, y=run_name, fill=run_name)) + ggtitle("Clock rate (SNP / site / year)\n") + scale_x_continuous(labels = scales::scientific) + 
            geom_density_ridges(scale=1.5, alpha=1) + scale_fill_manual(values = get_sequential_palette("YlGnBu", ncols)) + xlab("") + ylab("") + 
            theme_cowplot() + theme(
                panel.grid.minor = element_blank(),
                legend.position="none"
            ) 
    ...
    p4 <- ggplot(df, aes(x=InfectiousPeriod, y=run_name, fill=run_name)) + ggtitle("Infectious period (years)\n") +
        geom_density_ridges(scale=1.5, alpha=1) + scale_fill_manual(values = get_sequential_palette("Greens", ncols)) + xlab("") + ylab("") +
    ...
    p5 <- ggplot(df, aes(x=SamplingProportion, y=run_name, fill=run_name)) + ggtitle("Sampling proportion\n") +
        geom_density_ridges(scale=1.5, alpha=1) + scale_fill_manual(values = get_sequential_palette("Oranges", ncols)) + xlab("") + ylab("") +
        scale_x_continuous(breaks=seq(0, xmax, xstep)) +
        theme_cowplot() + theme(
            panel.grid.minor = element_blank(),
            legend.position="none"
    ...
    if ("ReproductionNumber" %in% colnames(df)){
        ...
        p6 <- ggplot(df, aes(x=ReproductionNumber, y=run_name, fill=run_name)) + ggtitle("Reproduction number\n") +
            geom_density_ridges(scale=1.5, alpha=1) + scale_fill_manual(values = get_sequential_palette("YlOrRd", ncols)) + xlab("") + ylab("") +
            scale_x_continuous(breaks=seq(0, ceiling(max(df$ReproductionNumber)), 1)) +
            geom_vline(xintercept = 1, linetype="dotted", size=0.5) +
            theme_cowplot() + theme(
                panel.grid.minor = element_blank(),
...
get_cosky_plot <- function(path=NULL, pattern=NULL, most_recent=NULL){
    
    
    files = list.files(path=path, pattern=pattern, full.names=TRUE)
    files = stringr::str_sort(files, numeric=TRUE)
    
    cosky_data <- lapply(files, function(f){
        
        data <- readLogfile(f, burnin=0.1) 
        sky <- getSkylineSubset(data,  "bPopSizes")
        hpd <- getMatrixHPD(sky, alpha=0.01)
        
        mean_age <- mean(data$TreeHeight) # 31.92
        mean_origin <- most_recent-mean_age # 1988.07
        
        timegrid <- 1:as.integer(mean_age)
        age_samples <- data$TreeHeight
        
        Ne_gridded <- gridSkyline(sky, age_samples, timegrid)
        Ne_gridded_hpd <- getMatrixHPD(Ne_gridded)
        
        skygrid <- t(Ne_gridded_hpd)
        
        skygrid <- cbind(
            skygrid, data.frame(year=mean_origin+timegrid)
        )
        
        colnames(skygrid) <- c('lower', 'mean', 'upper', 'year')
        
        name <- tools::file_path_sans_ext(basename(f))
        
        p <- ggplot(data=skygrid, aes(x=year, y=mean)) + geom_line(size=1) +
            geom_ribbon(aes(ymin=skygrid$lower, ymax=skygrid$upper), linetype=2, alpha=0.1)
        
        result <- list(
            df=data.frame(
                run_name=rep(name, length(data$TreeHeight)),
                tree_height=most_recent-data$TreeHeight, 
    ...
    posteriors <- lapply(cosky_data, function(x){return(x$df)})
    ne_plots <- lapply(cosky_data, function(x){return(x$ne_plot)})
    names <- lapply(cosky_data, function(x){return(x$name)})
    
    df <- do.call(rbind, posteriors) 
    
    df$run_name <- factor(
        df$run_name, levels=stringr::str_sort(
            levels(factor(df$run_name)), numeric=TRUE
        )
    )
    
    p1 <- ggplot(df, aes(x=tree_height, y=run_name, fill=run_name)) + ggtitle("MRCA\n") +
        geom_density_ridges(scale=1.5, alpha=0.80) + scale_fill_brewer(palette = "Purples")  + ylab("") + xlab("") + theme(legend.position="none")
    
    p2 <- ggplot(df, aes(x=clock_rate, y=run_name, fill=run_name)) + ggtitle("Clock rate\n") +  scale_x_continuous(labels = scales::scientific) +
        geom_density_ridges(scale=1.5, alpha=0.80) + scale_fill_brewer(palette = "Blues")  + ylab("") + xlab("") + theme(legend.position="none")
    
    p3 <- ne_plots[[2]] + ylab("Ne") + xlab("") + theme(legend.position="none") + ggtitle("d = 4") + theme_classic()
    p4 <- ne_plots[[3]] + ylab("Ne") + xlab("") + theme(legend.position="none") + ggtitle("d = 8") + theme_classic()
    
    po <- cowplot::plot_grid(p1, p2, p3, p4)
    cowplot::save_plot("cosky.pdf", po, base_asp=2.1, base_height=7.7)
